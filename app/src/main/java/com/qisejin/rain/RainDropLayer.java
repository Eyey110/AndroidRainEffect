...
public class RainDropLayer {
    ...
    static float vertexsData[] = {-1f, 1f, -1f, -1f, 1f, -1f, 1f, 1f,};
    ...
    public RainDropLayer(Context context, int width, int height, Bitmap srcImage) {
        ...
        indiceBuffer = ByteBuffer.allocateDirect(mIndicesData.length * 2).order(ByteOrder.nativeOrder()).asShortBuffer();
        ...
        mProgrammerHandle = OpenGLUtils.loadProgram(Utils.readShaderFromRawResource(context, R.raw.simple_vertex_shader), Utils.readShaderFromRawResource(context, R.raw.water_fragment_shader));
...
//        Bitmap fg = blur(context, scaled,10);
        ...
        mBackgroundTextureHandle = loadTexture(bg);
        mForegroundtextureHandle = loadTexture(fg);
        mDropShineHandle = loadTexture(context, R.drawable.drop_shine);
